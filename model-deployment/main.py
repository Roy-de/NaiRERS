...
async def predict(input_data: InputData):
    try:
        ...
        single_point = np.array([[input_data.Coordinates.x, input_data.Coordinates.y]])
...
async def get_neighbours(input_data: InputData):
    try:
        single_point = [input_data.Coordinates.x, input_data.Coordinates.y]
        ...
        distances_to_clusters = [haversine_distance(single_point, centroid)[0][1] / 1000 for centroid in
                                 cluster_centers]
        ...
        return {"distances_to_clusters": [int(distance) for distance in distances_to_clusters]}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
